...
WGPUSurface CreateWGPUSurface(const WGPUInstance& _instance, GLFWwindow* window);
...
bool InitWGPU(GLFWwindow* window)
{
    ...
    wgpu_surface = _instance.CreateSurface(&surface_desc).MoveToCHandle();
#else
    wgpu_surface = CreateWGPUSurface(_instance.Get(), window);
#endif
    if (!wgpu_surface)
        return false;
