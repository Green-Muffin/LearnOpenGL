...
VkPhysicalDevice ImGui_ImplVulkanH_SelectPhysicalDevice(VkInstance _instance)
{
    uint32_t gpu_count;
    VkResult err = vkEnumeratePhysicalDevices(_instance, &gpu_count, nullptr);
    check_vk_result(err);
...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance _instance, VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator, int width, int height, uint32_t min_image_count, VkImageUsageFlags image_usage)
{
    IM_ASSERT(g_FunctionsLoaded && "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
...
void ImGui_ImplVulkanH_DestroyWindow(VkInstance _instance, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator)
{
    IM_UNUSED(_instance);
