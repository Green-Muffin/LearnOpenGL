...
#define LOG_ERROR(msg) std::fprintf(stderr, "[RHI][ERROR] %s\n", msg)
#define LOG_WARN(msg) std::fprintf(stderr, "[RHI][WARN] %s\n", msg)

// https://gcc.gnu.org/onlinedocs/cpp/Stringizing.html
#define gfx_XSTR(s) gfx_STR(s)
#define gfx_STR(s) #s
...
namespace gfx
{
    ...
    {
        m_window = init_info.window;

        m_viewport = {0.0f, 0.0f, (float)init_info.window_width, (float)init_info.window_height, 0.0f, 1.0f};
        m_scissor  = {{0, 0}, {init_info.window_width, init_info.window_height}};
        ...
        // https://gcc.gnu.org/onlinedocs/cpp/Common-Predefined-Macros.html
#if defined(__linux__)
        char const* vk_layer_path = gfx_XSTR(gfx_VK_LAYER_PATH);
        setenv("VK_LAYER_PATH", vk_layer_path, 1);
#elif defined(__MACH__)
        // https://developer.apple.com/library/archive/documentation/Porting/Conceptual/PortingUnix/compiling/compiling.html
        char const* vk_layer_path    = gfx_XSTR(gfx_VK_LAYER_PATH);
        char const* vk_icd_filenames = gfx_XSTR(gfx_VK_ICD_FILENAMES);
        setenv("VK_LAYER_PATH", vk_layer_path, 1);
...
#else
    char vulkan_sdk_path[MAX_PATH] = {};
    DWORD vulkan_sdk_path_len = GetEnvironmentVariableA("VULKAN_SDK", vulkan_sdk_path, MAX_PATH);
    if (vulkan_sdk_path_len > 0 && vulkan_sdk_path_len < MAX_PATH)
    {
        std::string vk_layer_path = std::string(vulkan_sdk_path) + "\\Bin";
        SetEnvironmentVariableA("VK_LAYER_PATH", vk_layer_path.c_str());
    }
#endif
...
#error Unknown Compiler
    ...
    void VulkanRHI::createInstance()
    {
        ...
        {
            LOG_WARN("validation layers requested, but not available, disable validation layers");
            m_enable_validation_Layers = false;
            m_enable_debug_utils_label = false;
        }

        m_vulkan_api_version = VK_API_VERSION_1_3;  // Using Api Version 1_3

        // app info
        VkApplicationInfo appInfo {};
        appInfo.sType              = VK_STRUCTURE_TYPE_APPLICATION_INFO;
        appInfo.pApplicationName   = "Dodoe Renderer";
        appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
        appInfo.pEngineName        = "dodoe";
    ...
    void VulkanRHI::initializeDebugMessenger()
    {
        ...
        if (m_enable_validation_Layers)
        {
            ...
            {
                LOG_ERROR("failed to set up debug messenger!");     // MARK: USING ASSERT;
            }
        }
    ...
    void VulkanRHI::createWindowSurface()
    {
        ...
        {
            LOG_ERROR("glfwCreateWindowSurface failed!");   // MARK: USING ASSERT;
        }
    }
    ...
    void VulkanRHI::initializePhysicalDevice()
    {
        ...
        {
            LOG_ERROR("enumerate physical devices failed!");    // MARK: USING ASSERT;
        }
        else
        {
            ...
            if (m_physical_device == VK_NULL_HANDLE)    // MARK: USING ASSERT;
            {
    ...
    void VulkanRHI::createLogicalDevice()
    {
        ...
        {
            LOG_ERROR("vk create device");  // MARK: USING ASSERT;
        }
    ...
    void VulkanRHI::createCommandPool()
    {
        // default graphics command pool
        {
            ...
            {
                LOG_ERROR("vk create command pool");    // MARK: USING ASSERT;
            }
    ...
    void VulkanRHI::cmdBeginRenderPassPFN(RHICommandBuffer* commandBuffer, const RHIRenderPassBeginInfo* pRenderPassBegin, RHISubpassContents contents)
    {
        ...
        for (int i = 0; i < clear_value_size; ++i)
        {
            ...
            std::memcpy(&vk_clear_value_element, &rhi_clear_value_element, sizeof(VkClearValue));
    ...
    void VulkanRHI::createCommandBuffers()
    {
        ...
        for (uint32_t i = 0; i < k_max_frames_in_flight; ++i)
        {
            ...
            {
                LOG_ERROR("vk allocate command buffers");   // MARK: USING ASSERT;
            }
    ...
    RHISampler* VulkanRHI::getOrCreateDefaultSampler(RHIDefaultSamplerType type)
    {
        switch (type)
        {
        case gfx::Default_Sampler_Linear:
        ...
        case gfx::Default_Sampler_Nearest:
    ...
    {
        vkDestroyRenderPass(m_device, ((VulkanRenderPass*)renderPass)->getResource(), nullptr);
    }
    ...
    gfx::QueueFamilyIndices VulkanRHI::findQueueFamilies(VkPhysicalDevice physicalm_device) // for device and surface
    {
    ...
    gfx::SwapChainSupportDetails VulkanRHI::querySwapChainSupport(VkPhysicalDevice physicalm_device)
    {
    ...
    RHISwapChainDesc VulkanRHI::getSwapchainInfo()
    {
        ...
        desc.imageFormat = m_swapchain_image_format;
