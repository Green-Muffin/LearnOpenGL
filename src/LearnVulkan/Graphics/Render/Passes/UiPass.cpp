...
namespace LearnVulkan {
    ...
    void UiPass::initialize(gfx::RHI* rhi, GLFWwindow* window, gfx::RHIRenderPass* render_pass, uint32_t subpass) {
        ...
        if (rhi_ == nullptr || window_ == nullptr || render_pass_ == nullptr) {
            throw std::runtime_error("UiPass initialize failed: invalid input resources");
        }

        auto* vulkan_rhi = dynamic_cast<gfx::VulkanRHI*>(rhi_);
        ...
        init_info.QueueFamily = rhi_->getQueueFamilyIndices().graphics_family.value();
        init_info.Queue = static_cast<gfx::VulkanQueue*>(rhi_->getGraphicsQueue())->getResource();
    ...
    void UiPass::begin_frame() {
        ...
        ImGui::Text("Frame index: %u", static_cast<uint32_t>(rhi_->getCurrentFrameIndex()));
        ImGui::End();
